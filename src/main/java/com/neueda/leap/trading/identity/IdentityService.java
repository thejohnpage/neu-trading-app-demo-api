...
public class IdentityService {
    ...
    public ClientRegistrationResponse registerClient(ClientRegistrationRequest request) {
        ...
        String accountNumber =
                "ACC-" + accountId.toString().substring(0, 8).toUpperCase();
    ...
    private void validateRoles(List<String> roles) {
        if (roles == null || roles.isEmpty()) {
            throw new IllegalArgumentException(
                    "Administrative user requires at least one role");
        }

        if (roles.stream().anyMatch(role -> !ADMIN_ROLES.contains(role))) {
            throw new IllegalArgumentException(
                    "Unknown administrative role");
        }
    }

    private void requireUser(UUID id) {
        Integer count = jdbc.queryForObject(
                "SELECT COUNT(*) FROM identity.users WHERE user_id = ?",
                Integer.class,
                id);
