...
@Mapper public interface PricingDecisionRepository {
 ...
 @Select("SELECT pricing_decision_id pricingDecisionId,order_id orderId,instrument_id instrumentId,bid_price bidPrice,ask_price askPrice,execution_price executionPrice,quote_source quoteSource,quote_timestamp quoteTimestamp,priced_at pricedAt FROM audit.pricing_decisions WHERE order_id=#{orderId}") Optional<PricingDecision> findByOrderId(java.util.UUID orderId);
}
