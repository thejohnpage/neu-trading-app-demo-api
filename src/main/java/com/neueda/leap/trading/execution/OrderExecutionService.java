...
public class OrderExecutionService {
 ...
 public void execute(UUID orderId){
  ...
  BigDecimal executionPrice="BUY".equals(order.getSide())?quote.ask():quote.bid(); BigDecimal notional=order.getQuantity().multiply(executionPrice); Instant now=Instant.now();
  ...
  PositionId positionId=new PositionId(order.getAccountId(),order.getInstrumentId()); Position position=positions.findById(positionId).orElse(null);
  BigDecimal positionDelta="BUY".equals(order.getSide())?order.getQuantity():order.getQuantity().negate();
  if(position==null){if(positionDelta.signum()<0)throw new IllegalStateException("Cannot sell absent position");position=Position.create(order.getAccountId(),order.getInstrumentId(),positionDelta,now);positions.save(position);}else position.apply(positionDelta,now);
  fills.save(Fill.create(orderId,order.getQuantity(),executionPrice,now)); order.markFilled(now); events.save(OrderEvent.of(orderId,"PRICED",now)); events.save(OrderEvent.of(orderId,"FILLED",now));
 }
}
