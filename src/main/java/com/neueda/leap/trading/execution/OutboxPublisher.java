...
public class OutboxPublisher {
 ...
 public void publish(){for(OutboxEvent event:outbox.findUnpublished()){
   ...
   try{OrderAcceptedMessage message=json.readValue(event.getPayload(),OrderAcceptedMessage.class);
       kafka.send("order.accepted",message.orderId().toString(),event.getPayload()).get(); event.markPublished(Instant.now()); outbox.markPublished(event);}
