...
import org.springframework.context.annotation.*;import com.neueda.leap.trading.audit.AuditTrailFilter;import org.springframework.security.config.annotation.web.builders.HttpSecurity;import org.springframework.security.config.http.SessionCreationPolicy;import org.springframework.security.web.*;import org.springframework.security.web.authentication.UsernamePasswordAuthenticationFilter;import org.springframework.web.cors.*;
@Configuration public class SecurityConfig {
 @Bean SecurityFilterChain securityFilterChain(HttpSecurity http,AuthValidationFilter authFilter,AuditTrailFilter auditFilter)throws Exception{
  return http.csrf(c->c.disable()).cors(c->{}).formLogin(c->c.disable()).httpBasic(c->c.disable()).logout(c->c.disable()).requestCache(c->c.disable())
   ...
   .addFilterBefore(authFilter,UsernamePasswordAuthenticationFilter.class).addFilterAfter(auditFilter,AuthValidationFilter.class).build();
 }
