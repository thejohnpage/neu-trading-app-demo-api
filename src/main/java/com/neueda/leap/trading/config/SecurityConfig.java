 ...
 @Bean SecurityFilterChain securityFilterChain(HttpSecurity http,AuthValidationFilter authFilter,AuditTrailFilter auditFilter)throws Exception{
  return http.csrf(c->c.disable()).cors(c->{}).formLogin(c->c.disable()).httpBasic(c->c.disable()).logout(c->c.disable()).requestCache(c->c.disable())
   ...
   .authorizeHttpRequests(a->a
    ...
    .requestMatchers("/api/v1/admin/orders/**").hasAnyAuthority("ROLE_SUPER_ADMIN","ROLE_ADMIN_OPERATIONS","ROLE_TRADING_OPERATIONS","ROLE_RISK","ROLE_COMPLIANCE")
    .requestMatchers("/api/v1/admin/audit/**").hasAnyAuthority("ROLE_SUPER_ADMIN","ROLE_ADMIN_OPERATIONS","ROLE_COMPLIANCE","ROLE_RISK")
    .requestMatchers("/api/v1/admin/reports/**").hasAnyAuthority("ROLE_SUPER_ADMIN","ROLE_ADMIN_REPORTING","ROLE_ANALYST","ROLE_FINANCE")
