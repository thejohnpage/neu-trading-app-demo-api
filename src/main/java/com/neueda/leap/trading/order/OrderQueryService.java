...
public class OrderQueryService {
 ...
 public List<OrderResponse> findAll(){Set<UUID> ids=ownedAccountIds();if(ids.isEmpty())return List.of();return orders.findByAccountIds(ids).stream().map(OrderResponse::from).toList();}
 ...
 private Order ownedOrder(UUID id){Set<UUID> ids=ownedAccountIds();return orders.findOwned(id,ids).orElseThrow(() -> new ResourceNotFoundException("Order not found"));}
